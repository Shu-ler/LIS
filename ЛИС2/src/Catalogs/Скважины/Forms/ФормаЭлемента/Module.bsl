...
Процедура ПриборыУчетаПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		СтруктураПараметры = Новый Структура("Владелец, Организация", Объект.Ссылка, ВернутьОрганизацию(Объект.Владелец));
		// 1СКСУ - Казанцев И.В. 25.01.17 13:40
		...
		// 1СКСУ - Казанцев И.В. 25.01.17 13:40 Конец
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Необходимо сохранить Скважину");
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Функция ВернутьОрганизацию(Водозабор)
	
	Возврат Водозабор.Организация; 
