...
Процедура ПриОпределенииПсевдонимовОбработчиков(СоответствиеИменПсевдонимам) Экспорт
	...
	СоответствиеИменПсевдонимам.Вставить("МиграцияПриложений.УдалитьВременногоПользователя");
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.ОбщиеДанные = Истина;
	Обработчик.Версия    = "2.0.10.5";
	Обработчик.Процедура = "МиграцияПриложений.УдалитьВременныхПользователей";
	Обработчик.РежимВыполнения = "Оперативно";
...
Процедура ЗаданиеВыгрузка(ЗавершитьМиграцию = Ложь) Экспорт
	...
	Если СостояниеВыгрузки.НомерОтправленногоСообщения = 0 Тогда
		...
		ОшибкиВыгружаемыхДанных = ВыгрузкаЗагрузкаДанных.ОшибкиВыгружаемыхДанных(ТипыМоделиДанных, ТипыОбщихДанных);
		Если ЗначениеЗаполнено(ОшибкиВыгружаемыхДанных) Тогда
			Строки = Новый Массив;
			Строки.Добавить(НСтр("ru = 'Обнаруженные ошибки:'"));
			Строки.Добавить(Символы.ПС);
			Для Каждого ОшибкаВыгружаемыхДанных Из ОшибкиВыгружаемыхДанных Цикл
				Строки.Добавить("● ");
				Строки.Добавить(ОшибкаВыгружаемыхДанных);
				Строки.Добавить(Символы.ПС);
			КонецЦикла;
			ПредставлениеОшибки = СтрСоединить(Строки);
			ЗавершитьВыгрузкуСОшибкой(СостояниеВыгрузки, ПредставлениеОшибки, ПредставлениеОшибки);
			Попытка
				ОтменитьЗагрузкуВСервисе(ПараметрыДоступа);
			Исключение
				// Обработка исключения не требуется.
				// Загрузка отменится автоматически, но позже.
			КонецПопытки;
			Возврат;
		КонецЕсли;
	...
	ИначеЕсли СостояниеВыгрузки.Состояние = Перечисления.СостоянияМиграцииПриложения.ОжиданиеЗагрузки Тогда
		...
		КонецПопытки;
		Если Не СостояниеЗагрузки.ВыполняетсяЗагрузка Тогда
			ПередачаДанныхСервер.ПолучитьИзЛогическогоХранилища(ПараметрыДоступа, "migration", "remove-user");
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
Процедура ЗаданиеЗагрузка() Экспорт
	...
	Файл = Новый Файл(ФайлыБТС.ПолноеИмяФайлаВременногоХранилища(Выборка.ИмяФайла));
	...
	Попытка
		Если Результат.Успешно Тогда
			...
			ФайлыБТС.УстановитьСрокХраненияФайлаВременногоХранилища(Выборка.ИмяФайла, 60);
	...
	Попытка
		ФайлыБТС.УдалитьФайлВременногоХранилища(Выборка.ИмяФайла);
	Исключение
		ПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		РегистрацияОшибки(НСтр("ru = 'Загрузка'", ОбщегоНазначения.КодОсновногоЯзыка()), ПредставлениеОшибки);
	КонецПопытки;
...
Функция ПользователиСервиса(ИсточникПараметровДоступа) Экспорт
	...
	СписокПользователейСервиса = Новый Массив;
	Для Каждого ДанныеПользователя Из РезультатВызова.user Цикл
		...
		СписокПользователейСервиса.Добавить(ПользовательСервиса);
		
	КонецЦикла;
	
	Возврат СписокПользователейСервиса;
...
Функция Описание(ИдентификаторХранилища, ИдентификаторДанных) Экспорт
	...
	ИначеЕсли ИдентификаторДанных = "remove-user" Тогда
		ПараметрыЗадания = Новый Структура("ИмяМетода", "МиграцияПриложений.УдалитьВременногоПользователя");
		НайденныеЗадания = ОчередьЗаданий.ПолучитьЗадания(ПараметрыЗадания);
		Если НайденныеЗадания.Количество() = 0 Тогда
			ПараметрыМетода = Новый Массив;
			ПараметрыМетода.Добавить(ПользователиИнформационнойБазы.ТекущийПользователь().Имя);
			ПараметрыЗадания.Вставить("Использование", Истина);
			ПараметрыЗадания.Вставить("Параметры", ПараметрыМетода);
			ОчередьЗаданий.ДобавитьЗадание(ПараметрыЗадания);
		Иначе
			ОчередьЗаданий.ИзменитьЗадание(НайденныеЗадания[0].Идентификатор, Новый Структура("ЗапланированныйМоментЗапуска", ТекущаяДатаСеанса()));
		КонецЕсли;
	Иначе
...
Процедура УдалитьВременныхПользователей() Экспорт
	...
	Для Каждого ПользовательИБ Из ВсеПользователи Цикл
		Если ПользовательИБ.Имя = "migration" Тогда
			ОбластьДанных = Число(ПользовательИБ.РазделениеДанных.ОбластьДанныхОсновныеДанные);
			ЗаписьОбласть = РегистрыСведений.ОбластиДанных.СоздатьМенеджерЗаписи();
			ЗаписьОбласть.ОбластьДанныхВспомогательныеДанные = ОбластьДанных;
			ЗаписьОбласть.Прочитать();
			Если ЗаписьОбласть.Выбран() И ЗаписьОбласть.Статус = Перечисления.СтатусыОбластейДанных.Используется Тогда
...
Процедура УдалитьВременногоПользователя(Имя) Экспорт
	...
	Отбор = Новый Структура("ИмяМетода", "МиграцияПриложений.УдалитьВременногоПользователя");
	Для Каждого Задание Из ОчередьЗаданий.ПолучитьЗадания(Отбор) Цикл
...
Процедура ЗагрузитьСообщение(Источник)
	...
	ИмяФайлаВременногоХранилища = ФайлыБТС.НовыйФайлВременногоХранилища("AppMigrationIn", "zip", 60);
	ИмяФайла = ФайлыБТС.ПолноеИмяФайлаВременногоХранилища(ИмяФайлаВременногоХранилища);
	...
	Попытка
		...
		Запись.ИмяФайла = ИмяФайлаВременногоХранилища;
		НаборЗаписей.Записать(Ложь);

		ФайлыБТС.УстановитьСрокХраненияФайлаВременногоХранилища(ИмяФайлаВременногоХранилища, 60 * 24 * 2); // Двое суток
...
Функция СостояниеЗагрузки()
	...
	Если СостояниеЗагрузки.Выбран() Тогда
		...
		Если Не Статус.ВыполняетсяЗагрузка Тогда
			ПараметрыМетода = Новый Массив;
			ПараметрыМетода.Добавить(ПользователиИнформационнойБазы.ТекущийПользователь().Имя);
			ПараметрыЗадания = Новый Структура;
			ПараметрыЗадания.Вставить("ИмяМетода", "МиграцияПриложений.УдалитьВременногоПользователя");
			ПараметрыЗадания.Вставить("Использование", Истина);
			ПараметрыЗадания.Вставить("Параметры", ПараметрыМетода);
			ПараметрыЗадания.Вставить("ЗапланированныйМоментЗапуска", ТекущаяДатаСеанса() + 86400);
			ОчередьЗаданий.ДобавитьЗадание(ПараметрыЗадания);
		КонецЕсли;
	КонецЕсли;
...
Функция ЗагрузитьНаборЗаписей(ОбъектМетаданных, Идентификатор, ПараметрыЗагрузки)
	...
	Пока ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Цикл
		...
		Если ОбщегоНазначенияБТС.ЭтоРегистрСведений(ОбъектМетаданных)
			...
			ЗаписатьРегистрСведенийСУдалениемДублей(ОбъектМетаданных, Объект);
		Иначе
			Объект.Записать();
		КонецЕсли;
...
// Записывает в попытке набор записей регистра сведений, при неудачной записи
// выполняется очистка мешающих наборов и повторная запись.
// 
// Параметры:
//  ОбъектМетаданных - ОбъектМетаданныхРегистрСведений - Объект метаданных
//  НаборЗаписей - РегистрСведенийНаборЗаписей - Объект
//
Процедура ЗаписатьРегистрСведенийСУдалениемДублей(ОбъектМетаданных, НаборЗаписей)
	
	Попытка
		НаборЗаписей.Записать();
	Исключение
		ЗначенияИзмерений = Новый ТаблицаЗначений;
		Измерения = Новый Массив;
		...
		Если ОбъектМетаданных.ПериодичностьРегистраСведений <> Метаданные.СвойстваОбъектов.ПериодичностьРегистраСведений.Непериодический Тогда
			ЗначенияИзмерений.Колонки.Добавить(ОбъектМетаданных.СтандартныеРеквизиты.Период.Имя, ОбъектМетаданных.СтандартныеРеквизиты.Период.Тип);
			Измерения.Добавить(ОбъектМетаданных.СтандартныеРеквизиты.Период.Имя);
			УсловияСоединения.Добавить(СтрШаблон("ТаблицаРегистра.%1 = ЗначенияИзмерений.%1", ОбъектМетаданных.СтандартныеРеквизиты.Период.Имя));
		КонецЕсли;
		Для Каждого Измерение Из ОбъектМетаданных.Измерения Цикл
			ЗначенияИзмерений.Колонки.Добавить(Измерение.Имя, Измерение.Тип);
			Измерения.Добавить(Измерение.Имя);
			УсловияСоединения.Добавить(СтрШаблон("ТаблицаРегистра.%1 = ЗначенияИзмерений.%1", Измерение.Имя));
		КонецЦикла;
		Для Каждого Запись Из НаборЗаписей Цикл
			ЗаполнитьЗначенияСвойств(ЗначенияИзмерений.Добавить(), Запись);
		КонецЦикла;
		Запрос = Новый Запрос;
		...
		"ВЫБРАТЬ
		|	&Измерения
		|ПОМЕСТИТЬ ЗначенияИзмерений
		|ИЗ
		|	&ЗначенияИзмерений КАК ЗначенияИзмерений
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ЗначенияИзмерений КАК ЗначенияИзмерений
		|		ПО (&УсловияСоединения)
		|ГДЕ
		|	ТаблицаРегистра.Регистратор <> &Регистратор";
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Измерения", СтрСоединить(Измерения, ", "));
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловияСоединения", СтрСоединить(УсловияСоединения, " И "));
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТаблицаРегистра", ОбъектМетаданных.ПолноеИмя());
		Выборка = Запрос.Выполнить().Выбрать();
		Пока Выборка.Следующий() Цикл
			Дубли = РегистрыСведений[ОбъектМетаданных.Имя].СоздатьНаборЗаписей();
			Дубли.Отбор.Регистратор.Установить(Выборка.Регистратор);
			Дубли.ОбменДанными.Загрузка = Истина;
			Дубли.Записать();
		КонецЦикла;
		НаборЗаписей.Записать();
...
Функция ПреобразоватьНавигационнуюСсылкуВСтруктуру(Знач НавигационнаяСсылка)
	...
	ОбъектМетаданных = Метаданные.НайтиПоПолномуИмени(ЧастиПути[0] + "." + ЧастиПути[1]); // ОбъектМетаданныхСправочник
...
Процедура ДобавитьПолноправныхПользователейВГруппуАдминистраторыИОбновитьРолиОстальнымПользователям()
	...
	Если Не Результат.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			ПользовательОбъект = Выборка.Ссылка.ПолучитьОбъект(); // СправочникОбъект.Пользователи
...
Процедура УстановитьИспользованиеИтоговРегистров(Использование)
	...
	Для Каждого ОбъектМетаданных Из Метаданные.РегистрыНакопления Цикл
		Если Модель.Получить(ОбъектМетаданных) <> Неопределено Тогда
			Если ОбъектМетаданных.ВидРегистра = Метаданные.СвойстваОбъектов.ВидРегистраНакопления.Обороты
				И РегистрыНакопления[ОбъектМетаданных.Имя].ПолучитьРежимАгрегатов() Тогда
				РегистрыНакопления[ОбъектМетаданных.Имя].УстановитьИспользованиеАгрегатов(Использование);
			Иначе		
