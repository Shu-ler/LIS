...
Процедура ПриЗаполненииТиповИсключаемыхИзВыгрузкиЗагрузки(Типы) Экспорт
	...
	Типы.Добавить(Метаданные.РегистрыСведений.СведенияОПользователяхСервиса);
...
Процедура УдалитьОбласть0() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		СпрОбъект = Выборка.Ссылка.ПолучитьОбъект(); // СправочникОбъект.ОчередьЗаданий
...
// Устарела. Следует использовать см. ФайлыБТС.НовыйФайлВременногоХранилища
...
Функция КонтрольНеразделенныхДанныхПриОбновлении(ВызыватьИсключение = Истина) Экспорт
	...
	Если ОбъектыСНесколькимиРазделителями.Количество() > 0 Тогда
		...
		ТекстИсключенияСНесколькимиРазделителями = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			ТекстРазделителей, ТекстПроблемныеОбъекты);
	...
	Если ПустаяСтрока(Результат.ТекстИсключения) Тогда
...
Функция ИсключитьОбъектИзОчистки(Объект, ПараметрыОбъекта)
	...
	Если ПараметрыОбъекта.ЕстьПредопределенные Тогда
