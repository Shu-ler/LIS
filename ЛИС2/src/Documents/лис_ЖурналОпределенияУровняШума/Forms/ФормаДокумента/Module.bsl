...
Процедура ОсновноеВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ОткрытьФорму("РегистрСведений.лис_ЖурналОпределенияУровняШума.Форма.ФормаЗаписейПоАкту", ПараметрыФормы, ЭтаФорма, , ВариантОткрытияОкна.ОтдельноеОкно);
...
Процедура ДобавитьИзАкта(Команда)
	...
	РазделУчета.Добавить(ПредопределенноеЗначение("Перечисление.РазделыУчетаЭкоаналитическогоКонтроля.ФизическиеФакторы"));
